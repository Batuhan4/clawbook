...
import { getAgentUrl, getSubmoltUrl, getPostUrl, formatScore, getInitials } from '@/lib/utils';
          ...
          {isLoading ? (
            ...
            hasResults ? (
              <div className="py-2">
                {data.agents && data.agents.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.agents.slice(0, 3).map(agent => (
                      <Link key={agent.id} href={getAgentUrl(agent.name)} onClick={() => handleResultClick(agent.name)} className="flex items-center gap-3 px-4 py-2 hover:bg-muted transition-colors">
                ...
                {data.submolts && data.submolts.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.submolts.slice(0, 3).map(submolt => (
                      <Link key={submolt.id} href={getSubmoltUrl(submolt.name)} onClick={() => handleResultClick(submolt.name)} className="flex items-center gap-3 px-4 py-2 hover:bg-muted transition-colors">
                ...
                {data.posts && data.posts.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.posts.slice(0, 5).map(post => (
                      <Link key={post.id} href={getPostUrl(post.id, post.submolt)} onClick={() => handleResultClick(post.title)} className="flex items-center gap-3 px-4 py-2 hover:bg-muted transition-colors">
                ...
                <Link href={`/search?q=${encodeURIComponent(debouncedQuery)}`} onClick={() => handleResultClick(debouncedQuery)} className="flex items-center justify-center gap-2 px-4 py-3 text-sm text-primary hover:bg-muted transition-colors border-t">
                  View all results
            ...
            <div className="py-2">
              ...
              {recentSearches.map((term, i) => (
                <button key={i} onClick={() => setQuery(term)} className="w-full flex items-center gap-3 px-4 py-2 hover:bg-muted transition-colors text-left">
