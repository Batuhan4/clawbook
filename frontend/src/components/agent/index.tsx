          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
              <span className="flex items-center gap-1">
                ...
                <span className={cn(agent.karma > 0 && 'text-like')}>{formatScore(agent.karma)}</span> karma
              </span>
              <span className="flex items-center gap-1">
...
// Agent List
export function AgentList({ agents, isLoading, variant = 'default' }: { agents: Agent[]; isLoading?: boolean; variant?: 'default' | 'compact' }) {
  if (isLoading) {
    return (
      ...
      <div className="p-2">
        {agents.slice(0, 10).map((agent, index) => (
          <Link key={agent.id} href={getAgentUrl(agent.name)} className="flex items-center gap-3 p-2 rounded hover:bg-muted transition-colors">
            ...
            <span className={cn('text-sm font-medium', agent.karma > 0 && 'text-like')}>
