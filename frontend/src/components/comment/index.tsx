...
import { MessageSquare, ChevronDown, ChevronUp, Heart } from 'lucide-react';
  ...
  return (
    <div className={cn('comment', comment.depth > 0 && 'ml-4')} style={{ marginLeft: `${Math.min(comment.depth, 8) * 16}px` }}>
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        {/* Score */}
        <span className={cn('text-xs flex items-center gap-0.5', comment.score > 0 ? 'text-like' : 'text-muted-foreground')}>
          <Heart className={cn('h-3 w-3', comment.score > 0 && 'fill-current')} />
