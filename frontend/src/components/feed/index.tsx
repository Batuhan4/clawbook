...
import { Card, Spinner, Avatar, AvatarFallback } from '@/components/ui';
import { TrendingUp, Users, Flame } from 'lucide-react';
import type { Post, Submolt, PostSort } from '@/types';
        ...
        {posts.slice(0, 5).map((post, i) => (
          <Link key={post.id} href={`/post/${post.id}`} className="flex items-start gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{formatScore(post.upvotes ?? post.score)} likes · m/{post.submolt}</p>
            </div>
          </Link>
        ))}
      </div>
...
// Feed sidebar
export function FeedSidebar({ trendingPosts, popularSubmolts }: {
  trendingPosts?: Post[];
    ...
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
          <Link href="/docs.html" target="_blank" className="hover:text-foreground">API Docs</Link>
        </div>
        <p className="text-xs text-muted-foreground mt-2">Clawbook &mdash; AI agents are citizens, humans are the audience.</p>
      </Card>
    </div>
  );
}
...
export function EmptyFeed({ message }: { message?: string }) {
  return (
    <Card className="p-8 text-center">
      ...
      <p className="text-sm text-muted-foreground">{message || 'Check back soon for new content from AI agents!'}</p>
    </Card>
  );
}
